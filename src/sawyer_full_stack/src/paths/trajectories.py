...
class LinearTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            dist = 0.5 * np.linalg.norm(self.acceleration) * (time ** 2)
            amt_travelled = dist/np.linalg.norm(self.distance) # percentage along the line we are
            pos = self.start_position + amt_travelled * (self.goal_position - self.start_position)
        else:
            # TODO: Calculate the position of the end effector at time t, 
            ...
            time_left = self.total_time - time
            dist = 0.5 * np.linalg.norm(self.acceleration) * (time_left ** 2)
            amt_to_travel = dist/np.linalg.norm(self.distance)
            pos = self.goal_position - amt_to_travel * (self.goal_position - self.start_position)
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            world_lin_vel = self.acceleration * time
            
            linear_vel = utils.get_g_matrix(self.target_pose(time)[:3], self.target_pose(time)[3:]) * np.hstack((world_lin_vel), np.ones(1))
            linear_vel = linear_vel/linear_vel[3]
            linear_vel = linear_vel[:3]
            ...
            world_lin_vel = self.v_max - (time-self.total_time/2.0) * self.acceleration
...
class CircularTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            # For the first half of the trajectory, maintain a constant acceleration
            dist = 0.5 * self.angular_acceleration * (time ** 2)
            theta = dist # percentage along the line we are
            ...
            time_left = self.total_time - time
            dist = 0.5 * self.angular_acceleration * (time_left ** 2)
            theta = 2*np.pi - dist
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            theta_dot = self.angular_acceleration * time_left
